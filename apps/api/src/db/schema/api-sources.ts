 ...
 * Reddit) is registered here with its rate limit, active/inactive toggle,
 ...
 * - `is_active` toggle allows disabling a source without removing its config
 * - `market` column links each source to a specific market category
 ...
 * - Column names and types align with the `NewsSource` interface in `packages/types/src/news.ts`
 */
export const apiSources = pgTable("api_sources", {
  ...
  /** Market category this source covers — matches the `market` PostgreSQL enum */
  market: marketEnum("market").notNull(),

  /** Base URL for the data source (e.g., "https://finnhub.io/api/v1") */
  baseUrl: text("base_url").notNull(),

  /** Whether this source is currently active/enabled for polling. Defaults to true. */
  ...
  /** Record update timestamp. Automatically set on insert and update. */
