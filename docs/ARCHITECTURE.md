...
| `SafetyResultMessage` | Service Worker → Content | Safety report (RugCheck + GoPlus + honeypot) | Push safety analysis results for token cards |
| `AIAnalysisMessage` | Service Worker → Content | AI narrative, dimension scores, confidence level | Push LLM analysis results for display |
| `ExitAlertMessage` | Service Worker → Content | Exit trigger type, affected position, action | Alert user to triggered exit conditions |
...
| `https://price.jup.ag/*` | Host Permission | Jupiter API access |
